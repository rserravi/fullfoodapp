...
DEFAULTS = {"preheat_c": 200, "cook_c": 190, "flip_each_min": 5}
...
def compile_steps(steps: List[StepGeneric]) -> List[AppliancePlanStep]:
    ...
    if any(s.action == "cook" for s in steps):
        plan.append(AppliancePlanStep(action="preheat", temperature_c=DEFAULTS["preheat_c"], time_min=3,
    ...
    for s in steps:
        if s.action == "cook":
            ...
            plan.append(AppliancePlanStep(action="cook", temperature_c=t, time_min=time,
                                          instructions=f"Cocina a {t}°C durante {time} min. Agita a mitad."))
        elif s.action == "flip":
            plan.append(AppliancePlanStep(action="flip", temperature_c=None, time_min=None,
                                          instructions="Abre, agita/voltea y cierra; continúa el tiempo restante."))
            ...
            plan.append(AppliancePlanStep(action="rest", temperature_c=None, time_min=s.time_min or 2,
                                          instructions="Deja reposar."))
        elif s.action == "serve":
