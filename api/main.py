...
async def search_route(req: SearchRequest):
    ...
    out = [SearchHit(id=str(h.id), score=h.score, text=h.payload.get("text", ""), metadata={k: v for k, v in h.payload.items() if k != "text"}) for h in hits]
...
async def gen_recipe(req: RecipeGenRequest):
    ...
    with open("prompts/recipe_generation.md", "r", encoding="utf-8") as f:
        tmpl = f.read()
    prompt = (tmpl.replace("{{ingredients}}", ", ".join(req.ingredients))
